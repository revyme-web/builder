// SmokeRing — Code component template (Paper-derived WebGL shader: billowing ring of layered smoke).
//
...
export const SMOKE_RING_COMPONENT = `'use client';

/** @label "Smoke Ring" */
/** @comment "A billowing ring of layered smoke, drifting and churning around its own center. Animates in preview and on the live site." */
/** @defaultWidth 600 */
/** @defaultHeight 400 */
/** @controls {
  "colorBack": { "type": "color", "label": "Background", "default": "#000000" },
  "color1": { "type": "color", "label": "Smoke color", "default": "#ffffff" },
  "color2": { "type": "color", "label": "Second color", "default": "#ffffff" },
  "colorCount": { "type": "number", "label": "Colors used", "min": 1, "max": 2, "step": 1, "default": 1 },
  "radius": { "type": "number", "label": "Radius", "min": 0, "max": 1, "step": 0.05, "default": 0.25 },
  "thickness": { "type": "number", "label": "Thickness", "min": 0.05, "max": 1, "step": 0.05, "default": 0.65 },
  "innerShape": { "type": "number", "label": "Inner shape", "min": 0, "max": 1, "step": 0.05, "default": 0.7 },
  "noiseScale": { "type": "number", "label": "Noise scale", "min": 0.5, "max": 5, "step": 0.1, "default": 3 },
  "noiseIterations": { "type": "number", "label": "Detail", "min": 1, "max": 10, "step": 1, "default": 8 },
  "scale": { "type": "number", "label": "Zoom", "min": 0.1, "max": 3, "step": 0.05, "default": 0.8 },
  "speed": { "type": "number", "label": "Speed", "min": 0, "max": 3, "step": 0.1, "default": 0.5 }
} */
...
var __PAPER = (() => {
  ...
  // entries/smoke-ring.js
  var smoke_ring_exports = {};
  __export(smoke_ring_exports, {
...
\`;
  ...
  // ../paper-shaders/packages/shaders/src/shaders/smoke-ring.ts
  var smokeRingMeta = {
    maxColorCount: 10,
...
void main() {
  ...
  return __toCommonJS(smoke_ring_exports);
})();
...
function SmokeRing({
  colorBack = '#000000', color1 = '#ffffff', color2 = '#ffffff', colorCount = 1, radius = 0.25, thickness = 0.65, innerShape = 0.7, noiseScale = 3, noiseIterations = 8, scale = 0.8, speed = 0.5,
  ...props
}) {
  ...
  useEffect(() => {
    ...
    const colors = [color1, color2].slice(0, Math.max(1, Math.min(2, Math.round(colorCount))));
    mount.setUniforms(Object.assign({ u_fit: 1, u_rotation: 0, u_offsetX: 0, u_offsetY: 0, u_originX: 0.5, u_originY: 0.5, u_worldWidth: 0, u_worldHeight: 0 }, {
      ...
      u_radius: radius, u_thickness: thickness, u_innerShape: innerShape,
      u_noiseScale: noiseScale, u_noiseIterations: Math.round(noiseIterations)
    }));
  ...
  }, [colorBack, color1, color2, colorCount, radius, thickness, innerShape, noiseScale, noiseIterations, scale, speed, isStatic]);
...
export default withResponsiveProps(SmokeRing);
`;
