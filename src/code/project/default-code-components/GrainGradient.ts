// GrainGradient — Code component template (Paper-derived WebGL shader: grainy color fields in seven shape modes).
//
...
export const GRAIN_GRADIENT_COMPONENT = `'use client';

/** @label "Grain Gradient" */
/** @comment "Softly animated color fields rendered through heavy film grain — seven shape modes from corner washes to spheres. Animates in preview and on the live site." */
/** @defaultWidth 600 */
/** @defaultHeight 400 */
/** @controls {
  "shape": { "type": "select", "label": "Shape", "default": "corners", "options": [{"label":"Corners","value":"corners"},{"label":"Wave","value":"wave"},{"label":"Dots","value":"dots"},{"label":"Truchet","value":"truchet"},{"label":"Ripple","value":"ripple"},{"label":"Blob","value":"blob"},{"label":"Sphere","value":"sphere"}] },
  "colorBack": { "type": "color", "label": "Background", "default": "#000000" },
  "color1": { "type": "color", "label": "Color 1", "default": "#7300ff" },
  "color2": { "type": "color", "label": "Color 2", "default": "#eba8ff" },
  "color3": { "type": "color", "label": "Color 3", "default": "#00bfff" },
  "color4": { "type": "color", "label": "Color 4", "default": "#2a00ff" },
  "colorCount": { "type": "number", "label": "Colors used", "min": 1, "max": 4, "step": 1, "default": 4 },
  "softness": { "type": "number", "label": "Softness", "min": 0, "max": 1, "step": 0.05, "default": 0.5 },
  "intensity": { "type": "number", "label": "Intensity", "min": 0, "max": 1, "step": 0.05, "default": 0.5 },
  "noise": { "type": "number", "label": "Grain", "min": 0, "max": 1, "step": 0.05, "default": 0.25 },
...
var __PAPER = (() => {
  ...
  // entries/grain-gradient.js
  var grain_gradient_exports = {};
  __export(grain_gradient_exports, {
    fragmentShader: () => grainGradientFragmentShader,
    getShaderNoiseTexture: () => getShaderNoiseTexture,
    shapes: () => GrainGradientShapes,
...
\`;
  ...
  // ../paper-shaders/packages/shaders/src/shaders/grain-gradient.ts
...
void main() {
  ...
  // ../paper-shaders/packages/shaders/src/get-shader-noise-texture.ts
  ...
  return __toCommonJS(grain_gradient_exports);
})();
...
function GrainGradient({
  shape = 'corners', colorBack = '#000000', color1 = '#7300ff', color2 = '#eba8ff', color3 = '#00bfff', color4 = '#2a00ff', colorCount = 4, softness = 0.5, intensity = 0.5, noise = 0.25, scale = 1, speed = 1,
  ...props
}) {
  ...
  useEffect(() => {
    ...
    const colors = [color1, color2, color3, color4].slice(0, Math.max(1, Math.min(4, Math.round(colorCount))));
    mount.setUniforms(Object.assign({ u_fit: 1, u_rotation: 0, u_offsetX: 0, u_offsetY: 0, u_originX: 0.5, u_originY: 0.5, u_worldWidth: 0, u_worldHeight: 0 }, {
      u_scale: scale,
      u_colorBack: mount.parseColor(colorBack),
      u_colors: colors.map(mount.parseColor), u_colorsCount: colors.length,
      u_softness: softness, u_intensity: intensity, u_noise: noise,
      u_shape: __PAPER.shapes[shape] || 4
    }));
    const noiseImg = __PAPER.getShaderNoiseTexture();
    if (noiseImg) {
      noiseImg.decode().then(() => { if (!disposed) mount.setTexture('u_noiseTexture', noiseImg, false); }).catch(() => {});
    }
    mount.resize();
    if (isStatic) {
  ...
  }, [shape, colorBack, color1, color2, color3, color4, colorCount, softness, intensity, noise, scale, speed, isStatic]);
...
export default withResponsiveProps(GrainGradient);
`;
