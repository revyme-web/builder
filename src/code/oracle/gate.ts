// oracle/gate.ts — the ONE gate every AI write path goes through.
//
// Extracted from `ai/freeform/freeform-client.ts` (2026-09-18): the gate is
// engine, not a feature of the freeform loop. The in-app agent, the MCP
// bridge and the freeform loop all commit through this exact pair, so a file
// one surface would bounce can never be committed by another. Keeping it in
// `ai/` forced `code/oracle` -> `ai/` imports, which is backwards: the oracle
// is the lower layer.
//
//   gateTurnFiles(files, activePath)  -> violations (PURE, writes nothing)
//   commitTurnFiles(files)            -> written[] (side effects + ONE undo entry)
//
