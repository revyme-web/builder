// ScaleXYControl.tsx — Scale ToolAtom with uniform/individual toggle.
// Layout matches SpacingControl: top row = global input + toggle, bottom row = X/Y when individual.
// Uniform: writes `scale`. Individual: writes `scaleX` + `scaleY`.
...
function ScaleXYAtom() {
  ...
  // Auto-detect mode from props
  const hasIndividual = !!(allProps.scaleX || allProps.scaleY);
  ...
  useEffect(() => {
    if (!userToggledRef.current) {
      setShowIndividual(!!(allProps.scaleX || allProps.scaleY));
    }
  }, [allProps.scaleX, allProps.scaleY, allProps.scale]);

  const uniformVal = allProps.scale || '1';
  const xVal = allProps.scaleX || allProps.scale || '1';
  const yVal = allProps.scaleY || allProps.scale || '1';
        ...
        <div className="flex items-center border border-[var(--control-border)] cut-corners cut-border [--cut-border-color:var(--control-border)] overflow-hidden shrink-0">
          <button tabIndex={-1}
            onClick={() => {
              if (showIndividual) {
                ...
                onChangeMultiple({ scale: xVal, scaleX: '', scaleY: '' });
              }
            }}
          ...
          <button tabIndex={-1}
            onClick={() => {
              ...
              userToggledRef.current = true;
              onChangeMultiple({ scaleX: uniformVal, scaleY: uniformVal, scale: '' });
            }}
            className={`flex items-center justify-center h-7 w-7 transition-colors cursor-pointer ${
              showIndividual
                ? 'bg-[var(--button-secondary-bg)] text-[var(--text-primary)]'
...
export function ScaleXYControl({ mode = 'direct', ...mp }: AtomProps) {
  return (
    <UnifiedControlProvider property="scale" defaultValue="1" mode={mode} {...mp}>
      <div className="flex items-center justify-between w-full">
        <ControlLabel label="Scale" property="scale" plain={mode !== 'direct'} />
        <ScaleXYAtom />
      </div>
