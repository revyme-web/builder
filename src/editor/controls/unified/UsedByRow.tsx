...
export function UsedByRow({ binding }: { binding: ControlBinding }) {
  ...
  return (
    <button
      ...
      className={`w-full h-8 flex items-center justify-between px-2 bg-[var(--grid-line)] border border-[var(--control-border)] ${binding.onNavigate ? 'hover:border-[var(--accent)] cursor-pointer' : 'cursor-default'} rounded-[var(--radius-lg)] transition-colors text-xs`}
    >
